...
public class WeightedDirectedGraph
{
    ...
    public Path SPFA(int src, int dest)
    {
        ...
        long[] d = new long[vertices.Count];
        bool[] V = new bool[vertices.Count];
        Edge[] Paths = new Edge[vertices.Count];
        ...
        while (Q.Count != 0)
        {
            ...
            foreach (Edge e in u.adjacentEdges)
            {
                ...
                if (D < d[e.v.id])
                {
                    ...
                    if (!Q.Contains(e.v))
                        Q.Enqueue(e.v);
                    // Rescatar el path sirve este segmento de codigo
                    Paths[e.v.id] = e;
                }
            }
        }

        // FIN del ALGORITMO de wikipedia

        List<Edge> paths = new List<Edge>();
        Edge next = Paths[dest];
        Path mysol = new Path();

        mysol.start = vertices[src];
        mysol.end = vertices[dest];
        mysol.weight = d[dest];
        int tmp = 0;
